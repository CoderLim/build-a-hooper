...
export async function POST(req: Request) {
  try {
    ...
    const { slug, title, description, image, content, categories, authorName, status } = await req.json();
...
export async function PUT(req: Request) {
  try {
    await checkAdmin();
    const { id, slug, title, description, image, content, categories, authorName, status } = await req.json();
