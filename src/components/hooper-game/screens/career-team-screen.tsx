  ...
  return (
    <section className="flex flex-1 flex-col items-center justify-center gap-8 text-center">
      ...
      <p className="max-w-lg text-sm text-white/55">
        {m['game.career.subtitle']()}
      </p>

      {phase === 'idle' && (
        <>
          <p className="text-sm text-white/45">
            {m['game.career.spin_prompt']()}
          </p>
          <GameButton onClick={onStartSpin}>
            {m['game.career.spin_team']()}
          </GameButton>
        </>
      )}
      ...
      {phase === 'locked' && careerTeam && (
        <>
          <div className="rounded-3xl border border-orange-300/40 bg-orange-300/10 px-6 py-10 sm:px-12">
